...
public class S3FileSystemProvider extends FileSystemProvider {
    ...
    public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
        ...
        final SeekableByteChannel channel = new S3SeekableByteChannel(s3Path, options);
    ...
    public FileChannel newFileChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
        ...
        final S3FileSystem fileSystem = s3Path.getFileSystem();
        final Properties properties = fileSystem.getProperties();
        final boolean multipartEnabled = Boolean.parseBoolean(properties.getProperty(MULTIPART_UPLOAD_ENABLED, "false"));
        ...
        if (options.contains(StandardOpenOption.READ) ||
            ...
            if (multipartEnabled) {
                LOGGER.debug("Multipart upload does not support options [{}].", options);
            }
        ...
        final FileChannel channel = new S3MultipartUploadFileChannel(outputStream);
